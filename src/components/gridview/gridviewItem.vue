<template>
    <div class="pt-grid-view-item"
        :style="itemStyle"
        :class="mode"
        @mousedown.stop
        @contextmenu="handleContextMenu"
    >
        <div class="pt-grid-view-item-container" :class="{
                selected: selected
            }"
            @click="selectItem"
            @dblclick="openItem"
        >
            <template v-if="!showExtFields">
                <pt-icon v-if="value.icon" :iconName="value.icon" :type="value.iconType || 'svg'" :size="iconSize"></pt-icon>
                <span class="name">{{ value.name }}</span>
            </template>
            <template v-if="showExtFields">
                <template v-if="mode == 'detail'">
                    <div class="detail-name-field"
                        :style="{
                            width: columns[0].width + 'px',
                            minWidth: columns[0].minWidth + 'px'
                        }"
                    >
                        <pt-icon :iconName="value.icon" :size="iconSize" :type="value.iconType || 'svg'"></pt-icon>
                        <span class="name" :class="{normal: !selected, selected}" :title="value.name">{{ value.name }}</span>
                    </div>
                    <span v-for="(field, idx) in extFields" :key="value.name + idx"
                        class="ext-fields"
                        :style="{
