<template>
    ...
    >
        ...
        <el-scrollbar style="height: calc(100% - 32px)">
            ...
            >
                <template v-slot="{ node, data: { type, protocol, children, data } }">
					<span class="custom-tree-node" @dblclick.stop="handleHostOpen(data)">
						<n-space>
							<n-icon v-if="!children" type="svg" v-bind:name="data.config.system || 'linux'" size="18" />
            ...
            <el-empty v-else :description="t('home.host-manager.session-tree.no-session-data')" />
        </el-scrollbar>
        <!--编辑文件夹-->
