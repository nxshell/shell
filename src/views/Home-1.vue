<template>
    <div class="nx-layout-wrapper">
        <div v-if="!collapsed" class="nx-layout-left">
            <nx-menu :data="sessionConfigsTree || []" />
        </div>
        <div class="nx-layout-right" :style="{width: `calc(100% - ${collapsed ? 0 : 295}px)`}">
            <!--	toggle-bar		-->
            <div
                class="nx-layout-toggle-bar"
                :class="{'nx-layout-toggle-bar--collapsed': !collapsed}"
                @click="handlerCollapsed"
            >
                <div class="nx-layout-toggle-bar__top"></div>
                <div class="nx-layout-toggle-bar__bottom"></div>
            </div>
            <div v-if="sessionInstTabs.length !== 0" class="nx-content-tabs">
                <tab-bar :data="sessionInstTabs" />
            </div>
            <div class="nx-content" :style="{height: `calc(100% - ${sessionInstTabs.length === 0 ? '0px' : '40px'})`}">
...
import {getSessionIcon, getSystemIcon, treeIconFilter} from '@/views/sysicons'
...
export default {
    name: 'Home',
    components: {Space, PtPopper, PtScrollContainer, TabBar, NxMenu},
    data() {
        return {
            collapsed: false,
            props: {
