<template>
    <div class="xterm-instance" :style="backgroundColor" @mousedown.middle="handleSelectPaste">
        <pt-toolbar :height="48">
            <pt-icon iconName="copy" slot="left" size="small" :title="T('home.session-instance.duplicate-session')" flat @click="copySession" v-show="toolbarShow"/>
            <pt-icon iconName="refresh" slot="left" size="small" :title="T('home.session-instance.reconnect')" flat @click="reconSession" v-show="toolbarShow" />
            <pt-inputbox type="text"
                ...
                slot="center"
            />
            <pt-icon iconName="ftp" slot="right" size="small" flat :title="T('home.session-instance.SFTP')" @click="openSFTP" v-show="toolbarShow"/>
            <pt-icon iconName="redirect" slot="right" size="small" flat :title="tunnelTitle" @click="openTunnel" v-show="toolbarShow"/>
        </pt-toolbar>
        <pt-xterm class="xterm-pt" ref="xterm" :sendToAllTerm="keyboradToAll" @sendToAll="openSendAll" @line-data="handleLog" @file-drop="handleFileDrop" @link="openLink" @key="onXtermKey" @termdata="onXtermData" @resize="onXtermResize" @titleChange="onTitleChange" @shortcut="handleShortCutEvent" v-context-menu="xtermMenu" :options="options" />
...
export default {
    ...
    methods: {
        ...
        send_many() {
            this.setKeyboradToAll(true);
        },
        send_one() {
            this.setKeyboradToAll(false);
