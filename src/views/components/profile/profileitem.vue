<template>
    <div class="pt-profile-item" v-if="show" :class="{simple: simple}">
        <template v-if="!simple">
            ...
            <pt-row>
                <pt-col :span="8">
                    <pt-inputbox v-model="editValue" v-if="type === 'text'" type="text"/>
                    <pt-inputbox v-model="editValue" v-if="type === 'number'" type="text"/>
                    <pt-inputbox v-model="editValue" v-if="type === 'password'" type="password"/>
                    <pt-file v-model="editValue" type="text" v-if="type === 'file'"/>
                    <pt-folder v-model="editValue" type="text" v-if="type === 'folder'"/>
                    <pt-select v-model="editValue" v-if="type === 'select'">
                        <pt-select-option v-for="(opt, idx) in options" :key="idx"
                            :label="T(opt.label)"
                            :value="opt.value"
                        />
                    </pt-select>
